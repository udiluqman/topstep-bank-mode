const BANKMODE_VERSION = "v12.5-ritual3m-ringfix";
...
// Storage
// ============================
const LS_KEY = "topstep_bankmode_v12_5";
...
function loadState(){
  ...
  try {
    ...
    // v12.3 migrations
    if(!s.ritualDoneFor) s.ritualDoneFor = "";
    if(!s.debrief) s.debrief = {active:false,t:60};
    if(!s.ritual) s.ritual = {active:false,t:180};
    if(!s.dayClosedFor) s.dayClosedFor = "";
...
window.addEventListener("hashchange", ()=>render(guardedRoute(route())));
...
function ritualModal(){
  ...
  const t = state.ritual.t ?? 10;
  const phases = ["INHALE","HOLD","EXHALE","HOLD","COMMIT"];
  const idx = state.ritual.phaseIdx ?? 0;
  const p = Math.max(0, Math.min(100, (10 - t) * 10));
  const showCommit = t <= 2;
  return `
        ...
        <div class="ritualWrap" style="margin-top:14px;">
          <div class="ritualProg" id="ritualProg" style="--p:${p};">
            <div class="ritualInner">
              <div class="phaseWord" id="ritualPhase">${phases[idx] || "INHALE"}</div>
              <div class="ritualNum" id="ritualNum">${t}</div>
            </div>
          </div>
          <div class="ritualSub">One trade max.</div>
...
function openRitual(){
  state.ritual = state.ritual || { active:false, t:10, lastDone:"", phaseIdx:0, phaseT:2 };
  state.ritual.active = true;
  state.ritual.t = 10;
  state.ritual.phaseIdx = 0;
  state.ritual.phaseT = 2;
  saveState();
  render(guardedRoute(route()));
  startRitualTimer();
}
...
function startRitualTimer(){
  // 5 phases x 2 seconds = 10 seconds
  const phases = ["INHALE","HOLD","EXHALE","HOLD","COMMIT"];
  window.__ritualTimer && clearInterval(window.__ritualTimer);

  const paint = ()=>{
    const numEl = document.getElementById("ritualNum");
    const phaseEl = document.getElementById("ritualPhase");
    const progEl = document.getElementById("ritualProg");
    const commitEl = document.getElementById("ritualCommit");
    const t = state.ritual?.t ?? 10;
    const idx = state.ritual?.phaseIdx ?? 0;

    if(numEl) numEl.textContent = String(t);
    if(phaseEl){
      phaseEl.textContent = phases[idx] || "INHALE";
      phaseEl.classList.remove("pop"); void phaseEl.offsetWidth; phaseEl.classList.add("pop");
    }
    if(progEl){
      const p = Math.max(0, Math.min(100, (10 - t) * 10)); // 0..100
      progEl.style.setProperty("--p", String(p));
    }
    if(commitEl){
      commitEl.style.display = (t <= 2) ? "inline-flex" : "none";
    }
  };
  ...
  window.__ritualTimer = setInterval(()=>{
    ...
    // If countdown already finished, wait for commit tap.
    if((state.ritual.t ?? 0) <= 0){ paint(); return; }

    state.ritual.t = Math.max(0, (state.ritual.t||0) - 1);

    state.ritual.phaseT = (state.ritual.phaseT ?? 2) - 1;
    if(state.ritual.phaseT <= 0){
      state.ritual.phaseIdx = Math.min(4, (state.ritual.phaseIdx ?? 0) + 1);
      state.ritual.phaseT = 2;
    }
    ...
    // When reaches 0, stop timer but do not complete automatically.
    if(state.ritual.t<=0){
      clearInterval(window.__ritualTimer);
      // Force COMMIT phase and show button
      state.ritual.phaseIdx = 4;
      saveState();
      paint();
    }
  }, 1000);
...
function todayETStr(){ return nowET().toISOString().slice(0,10); }
function isWeekdayET(){ const d=nowET().getDay(); return d>=1 && d<=5; }
function inRTH(){ const d=nowET(); const mins=d.getHours()*60+d.getMinutes(); return mins>=(9*60+30) && mins<=(16*60); }

function ritualRequiredNow(){
  const done = state.ritualDoneFor === todayETStr();
  const hasDecision = state.dayDecision && state.dayDecision.date===todayETStr() && state.dayDecision.made;
  const hasTrade = (state.trades||[]).some(t => (t.date||"") === todayETStr());
  return isWeekdayET() && inRTH() && !done && !hasDecision && !hasTrade && (state.dayClosedFor !== todayETStr());
}

function openRitual(){
  state.ritual = state.ritual || {active:false,t:180};
  state.ritual.active = true;
  state.ritual.t = 180;
  saveState();
  render(guardedRoute(route()));
...
function ritualCommit(){
  if(!(state.ritual && state.ritual.active)) return;
  if((state.ritual.t ?? 180) > 15){ toast("Wait for COMMIT"); return; }
  state.ritual.active = false;
  state.ritualDoneFor = todayETStr();
  saveState();
  toast("Token unlocked ✅");
  render(guardedRoute(route()));
}

function closeRitual(){
  window.__ritualTimer && clearInterval(window.__ritualTimer);
  if(state.ritual) state.ritual.active = false;
  saveState();
  render(guardedRoute(route()));
}
...
function startRitualTimer(){
  ...
  const paint = () => {
    ...
    if(!root) return;
    // re-render modal only (avoid full render thrash)
    root.innerHTML = ritualModalHTML() + debriefModalHTML();
  };
  paint();
  window.__ritualTimer = setInterval(() => {
    if(!(state.ritual && state.ritual.active)){ clearInterval(window.__ritualTimer); return; }
    ...
    paint();
    if(state.ritual.t <= 0){
      // if user didn't commit, keep modal until they tap commit
      clearInterval(window.__ritualTimer);
      paint();
    }
  }, 1000);
...
function openDebrief(){
  state.debrief = state.debrief || {active:false,t:60};
  state.debrief.active = true;
  state.debrief.t = 60;
  saveState();
  render(guardedRoute(route()));
  startDebriefTimer();
...
function debriefMark(ok){
  state.behavior = state.behavior || {days:{}};
  state.behavior.days = state.behavior.days || {};
  state.behavior.days[todayETStr()] = ok ? "green" : "red";
  saveState();
  toast(ok ? "Rules kept ✅" : "Rules broken ❌");
  render(guardedRoute(route()));
}
...
function startDebriefTimer(){
  ...
  window.__debriefTimer = setInterval(() => {
    ...
    state.debrief.t = Math.max(0, (state.debrief.t ?? 60) - 1);
    saveState();
    paint();
  }, 1000);
}

function ritualModalHTML(){
  if(!(state.ritual && state.ritual.active)) return "";
  ...
  <div class="modal" onclick="closeRitual()">
    <div class="modalCard" onclick="event.stopPropagation()">
      <div class="row" style="width:100%;justify-content:space-between;align-items:center;">
        <div class="title" style="margin:0">Bank Ritual</div>
        <button class="btn" style="padding:8px 10px" onclick="closeRitual()">✕</button>
      </div>
      <div class="ring ${showCommit ? "pulseGlow" : ""}" style="--p:${p};">
        <div class="ringInner">
          <div class="phaseWord">${phase}</div>
          <div class="bigNum">${t}</div>
          <div class="subtle">Calm first → then action.</div>
        </div>
      </div>
      <div class="muted" style="text-align:center;max-width:280px">
        3 minutes. No thinking. Only regulation.
      </div>
      <button class="btn btn-primary" style="width:100%;display:${showCommit ? "inline-flex":"none"}" onclick="ritualCommit()">
        I PROTECT CAPITAL
      </button>
    </div>
  </div>`;
...
function debriefModalHTML(){
  if(!(state.debrief && state.debrief.active)) return "";
  const t = state.debrief.t ?? 60;
  const p = Math.max(0, Math.min(100, ((60 - t) / 60) * 100));
  const phase = (t > 40) ? "EXHALE" : (t > 20) ? "ACCOUNT" : "CLOSE";
  const showAcc = (t <= 40 && t > 20);
  const showClose = (t <= 20);
  return `
    ...
    <div class="modalCard" onclick="event.stopPropagation()">
      <div class="ring" style="--p:${p};">
        <div class="ringInner">
          <div class="phaseWord">${phase}</div>
          <div class="bigNum">${t}</div>
          <div class="subtle">Close the day cleanly.</div>
        </div>
      </div>
      <div style="display:${showAcc ? "flex":"none"};gap:10px;">
        <button class="btn btn-primary" onclick="debriefMark(true)">✅</button>
        <button class="btn btn-danger" onclick="debriefMark(false)">❌</button>
      </div>
      <button class="btn btn-primary" style="display:${showClose ? "inline-flex":"none"}" onclick="dayClosed()">DAY CLOSED</button>
      <button class="btn" onclick="closeDebrief()">Close</button>
    </div>
  </div>`;
}
...
function guardedRoute(r){
  const needs = ritualRequiredNow();
  if(needs && r !== "home"){
    toast("Ritual required.");
    return "home";
  }
  return r;
}
