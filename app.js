const BANKMODE_VERSION = "v12.2-ritual3m";
...
// Storage
// ============================
const LS_KEY = "topstep_bankmode_v12_2";
...
function render(view){
  ...
  try{
    if(view==="log") return app.innerHTML = pageLog() + ritualModal() + debriefModal() + ritualModal();
    if(view==="newtrade") return app.innerHTML = pageNewTrade() + ritualModal() + debriefModal() + ritualModal();
    if(view==="scale") return app.innerHTML = pageScale() + ritualModal() + debriefModal() + ritualModal();
    if(view==="behavior") return app.innerHTML = pageBehavior() + ritualModal() + debriefModal() + ritualModal();
    if(view==="protect") return app.innerHTML = pageProtect() + ritualModal() + debriefModal() + ritualModal();
    return app.innerHTML = pageHome() + ritualModal() + debriefModal() + ritualModal();
  }catch(err){
    console.error(err);
    app.innerHTML = `
      <div class="wrap">
...
function pageHome() + ritualModal() + debriefModal(){
  const ttd = tradesTodayCount();
...
function pageProtect() + ritualModal() + debriefModal(){
  return `
    <div class="card">
...
function pageLog() + ritualModal() + debriefModal(){
...
function pageNewTrade() + ritualModal() + debriefModal(){
  const ttd = tradesTodayCount();
...
function pageBehavior() + ritualModal() + debriefModal(){
...
function pageScale() + ritualModal() + debriefModal(){
...
function tradesTodayCount(){ const d=todayETStr(); const arr=state.trades||[]; return arr.filter(t=>(t.date||"")===d).length; }
...
function homeRitualCard(){
  ...
  const done = state.ritualDoneFor === todayETStr();
  const label = done ? "Ritual Done ✅" : (req ? "Start (3m)" : "Start (3m)");
  ...
  return `
    <div class="card">
      <div class="row" style="justify-content:space-between;align-items:center;">
        <div>
          <div class="title">BANK RITUAL</div>
          <div class="muted">${req ? "Unlock token. Calm → act." : "Session key."}</div>
        </div>
