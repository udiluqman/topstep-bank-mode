...
function pageHome(){
  ...
  if(state.protect.active && state.protect.date===todayStr()) tokenState = "LOCKED";
  ...
  const tokenClass = tokenState==="AVAILABLE" ? "good" : (tokenState==="USED" ? "warn" : (tokenState==="KEY" ? "warn" : "bad"));
    ...
    <div class="page">
      ...
      <div class="grid">
        <div class="card big">
          ...
          <div class="token ${tokenClass}">${tokenState==="KEY" ? "LOCKED" : tokenState}</div>
          ...
          ${tokenState==="AVAILABLE" ? `<div class="row" style="margin-top:10px;gap:10px;flex-wrap:wrap;">
            <button class="btn btn-primary" onclick="makeDayDecision('trade')">Trade Decision ✅</button>
            <button class="btn btn-danger" onclick="makeDayDecision('skip')">Skip Day ❌</button>
          </div>` : ``}
        </div>
...
function makeDayDecision(type){
  state.dayDecision = state.dayDecision || { date:"", made:false, type:"" };
  state.dayDecision.date = todayStr();
  state.dayDecision.made = true;
  state.dayDecision.type = type;
  saveState();
  if(type==="skip"){
    toast("Skip locked. Bank closed ✅");
    // optional: mark behavior green for compliance
    try{
