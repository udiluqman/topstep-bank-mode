window.__BM_ERROR=null;
window.onerror=function(msg,src,line,col,err){
  window.__BM_ERROR={msg:String(msg),src:String(src||""),line:line||0,col:col||0,stack:err&&err.stack?String(err.stack):""};
  try{const el=document.getElementById("app"); if(el){el.innerHTML=`<div style="padding:16px;"><div style="font-weight:800;margin-bottom:8px;">App Error</div><pre style="white-space:pre-wrap;background:#fff;border:1px solid #e6eaf0;padding:12px;border-radius:12px;">${window.__BM_ERROR.msg}\n${window.__BM_ERROR.src}:${window.__BM_ERROR.line}:${window.__BM_ERROR.col}\n\n${window.__BM_ERROR.stack}</pre><div style="margin-top:12px;color:#627D98;">Tip: if this happened after an update, open in an incognito tab once.</div></div>`;}}
  catch(e){}
};
const APP_VERSION = "v12-final-conditioning";


function navHeader(title){
  // Minimal, dependency-free header used across pages (prevents white-screen if refactor changes).
  return `
    <div class="container" style="padding-top:18px;padding-bottom:0;">
...
// Storage
// ============================
const LS_KEY = "topstep_bankmode_v12_final_conditioning";
const state = loadState();

function ensureBehaviorDays() {
  if (!state.behavior) {
    state.behavior = { days: {} };
    return;
  }
  if (!state.behavior.days) {
    const d = {};
    for (const k in state.behavior) {
      if (k !== 'days') d[k] = state.behavior[k];
    }
    state.behavior = { days: d };
  }
}
...
function loadState(){
  ...
  try {
    const raw = JSON.parse(localStorage.getItem(LS_KEY)) || defaults;
    // Normalize aggressively so old/broken storage never crashes render.
    const s = (typeof raw === 'object' && raw !== null) ? raw : structuredClone(defaults);

    // migrations / compatibility shims
    if(typeof s.meta !== 'object' || s.meta === null) s.meta = structuredClone(defaults.meta);
    if(typeof s.protect !== 'object' || s.protect === null) s.protect = structuredClone(defaults.protect);
    if(!Array.isArray(s.log)) s.log = [];
    if(typeof s.behavior !== 'object' || s.behavior === null) s.behavior = { days: {} };

    // (legacy) news block is still supported, but optional.
    if(typeof s.news !== 'object' || s.news === null) s.news = {bufferMin: 10, events: [], apiUrl: '', lastSyncET: '', autoEnabled: true};
    ...
    if(!s.behavior.days){
      const d = {};
      for (const k in s.behavior) {
        if (k !== 'days') d[k] = s.behavior[k];
      }
      s.behavior = { days: d };
    }
    if(typeof s.meta.pre !== 'object' || s.meta.pre === null) s.meta.pre = structuredClone(defaults.meta.pre);
    if(typeof s.meta.exec !== 'object' || s.meta.exec === null) s.meta.exec = structuredClone(defaults.meta.exec);
    if(typeof s.meta.notif !== 'object' || s.meta.notif === null) s.meta.notif = structuredClone(defaults.meta.notif);
    return s;
  }
...
function ritualRequiredNow(){
  // One ritual per ET day, only relevant during the trade window session.
  const rth = nowInTradeWindowET();
  if(!rth.inSession) return false;
  const key = todayKeyET();
  const done = !!(state.ritual && state.ritual.doneKey === key);
  return !done;
}
function nowInTradeWindowET(){
  ...
  // Trade window for this strategy (ET).
  // Default: first 2 hours of RTH (09:30–11:30 ET).
  const startToday = etToInstant(y,m,d,9,30);
  const stopToday  = etToInstant(y,m,d,11,30);

  const isWeekday = isETWeekday(now);
  // Full RTH session (09:30–16:00 ET) used for some gates/messages.
  const rthStart = etToInstant(y,m,d,9,30);
  const rthStop  = etToInstant(y,m,d,16,0);
  const inSession = (now >= rthStart && now <= rthStop) && isWeekday;

  return {
    inWindow: (now >= startToday && now <= stopToday) && isWeekday,
    inSession,
    startToday,
    stopToday,
    isWeekday,
...
function nowInRTHSessionET(){
  // RTH = 09:30–16:00 ET, Mon–Fri
  const now = new Date();
  // Convert to ET by formatting then re-parsing in that tz
  const parts = new Intl.DateTimeFormat('en-US', { timeZone:'America/New_York', year:'numeric', month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit', hour12:false, weekday:'short' }).formatToParts(now);
  const get = (t)=> parts.find(p=>p.type===t)?.value;
  const wd = get('weekday'); // Mon, Tue, ...
  const hh = parseInt(get('hour')||'0',10);
  const mm = parseInt(get('minute')||'0',10);
  const isWeekend = (wd === 'Sat' || wd === 'Sun');
  if(isWeekend) return { inSession:false, label:'Weekend Lock', reason:'Weekend' };
  const mins = hh*60+mm;
  const start = 9*60+30;
  const end   = 16*60; // inclusive end-ish
  const inSession = (mins >= start && mins <= end);
  return { inSession, label: inSession ? 'RTH Open' : 'Outside RTH', reason: inSession ? 'In session' : 'Outside session' };
}
...
function render(view){
  ...
  try{
    if(view==="log") return app.innerHTML = pageLog() + ritualModal() + debriefModal();
    if(view==="newtrade") return app.innerHTML = pageNewTrade() + ritualModal() + debriefModal();
    if(view==="scale") return app.innerHTML = pageScale() + ritualModal() + debriefModal();
    if(view==="behavior") return app.innerHTML = pageBehavior() + ritualModal() + debriefModal();
    if(view==="protect") return app.innerHTML = pageProtect() + ritualModal() + debriefModal();
    return app.innerHTML = pageHome() + ritualModal() + debriefModal();
  }catch(err){
    console.error(err);
    app.innerHTML = `
      <div class="wrap">
...
function homeControls(){
  const closed = isDayClosedToday();
  const win = nowInTradeWindowET();
  const needRitual = win.isWeekday && win.inSession && ritualRequiredNow();
  const decisionMade = state.dayDecision && state.dayDecision.date===todayETStr() && state.dayDecision.made;

  if(closed){
    return `<div class="card"><div class="title">BANK CLOSED</div><div class="muted">No actions today.</div></div>`;
  }

  if(needRitual){
    return `<div class="card"><div class="title">RITUAL REQUIRED</div><button class="btn btn-primary pulse" onclick="openRitual()">Start 3‑Min Ritual</button></div>`;
  }

  if(decisionMade){
    return `<div class="card"><div class="title">DECISION MADE</div><div class="muted">No further actions.</div></div>`;
  }

  return `<div class="card">
    <div class="title">TODAY DECISION</div>
    <div class="row" style="gap:10px;flex-wrap:wrap;margin-top:10px;">
      <button class="btn btn-primary" onclick="makeDayDecision('trade')">Trade ✅</button>
      <button class="btn btn-danger" onclick="makeDayDecision('skip')">Skip ❌</button>
    </div>
  </div>`;
}

function pageHome(){
  const w = nowInTradeWindowET();
  const badge = w.isWeekday ? (w.inSession ? "RTH Window" : "Outside Window") : "Weekend Lock";
  const net = (state.withdraw && typeof state.withdraw.net === "number") ? state.withdraw.net : 0;
  const pct = Math.min(100, Math.max(0, (net/500)*100));
  return `
    ${navHeader("Home")}
    <div class="container">
      <div class="card">
        <div class="row" style="justify-content:space-between;align-items:center;">
          <div>
            ...
            <div class="muted">Topstep $150K (Standard) • ES • 2m • RTH</div>
          </div>
      ...
      <div class="card">
        <div class="label">WITHDRAWAL CYCLE</div>
        <div class="row" style="justify-content:space-between;align-items:flex-end;">
          <div class="big">$${net}</div>
          <div class="muted">toward $500</div>
        </div>
        <div class="bar"><div class="barFill" style="width:${pct}%;"></div></div>
        <div class="row" style="justify-content:space-between;">
          <div class="muted">${Math.round(pct)}%</div>
          <div class="muted">Boring = profitable</div>
        </div>
      </div>

      <div class="card">
        <div class="label">TODAY RULE</div>
        <div><b>If missed entry</b> → skip day<br/><span class="muted">No second trade. Ever.</span></div>
      </div>
    </div>
    ${bottomNav("home")}
  `;
...
function ritualModal(){
  ...
  <div class="modal" onclick="closeRitual()">
    <div class="modalCard ritualSquare" onclick="event.stopPropagation()">
      <div class="ritualProg" id="ritualProg">
        <div class="ritualInner">
          <div class="phaseWord" id="ritualPhase">RESET</div>
          <div class="muted" id="ritualMantra" style="font-size:12px;margin-top:-2px;">Exhale longer than inhale.</div>
          <div class="ritualNum" id="ritualNum">180</div>
        </div>
      </div>
      <button id="ritualCommit" class="btn btn-primary" style="display:none;margin-top:10px" onclick="ritualCommit()">I PROTECT CAPITAL</button>
    </div>
  </div>`;
}
...
function pageLog(){
  const win = nowInTradeWindowET();
  const dec = (state.dayDecision && state.dayDecision.date === todayETStr()) ? state.dayDecision : null;
  const canTradeLog = !!(dec && dec.action === "trade" && win.inWindow);
  ...
  return `
    <div class="card">
      ...
      <div class="row" style="margin-top:12px;gap:10px;flex-wrap:wrap;">
        <button class="btn primary" onclick="attemptNewTrade()" ${canTradeLog ? "" : "disabled"}>+ Trade</button>
...
function pageNewTrade(){
  const rth = nowInRTHSessionET();
  const win = nowInTradeWindowET();
  const dec = (state.dayDecision && state.dayDecision.date === todayETStr()) ? state.dayDecision : null;

  // Hard gate: no trade logging outside RTH.
  if(!rth.inSession){
    return `
      <div class="card">
        <div class="kicker">LOCKED</div>
        <div class="h2">Outside RTH</div>
        <div class="sub">Logging is disabled outside 09:30–16:00 ET.</div>
        <div class="row" style="margin-top:12px">
          <button class="btn" onclick="location.hash='home'">Back</button>
        </div>
      </div>
    `;
  }

  // Soft gate: require a "TRADE" decision for today (prevents bypass via Log tab).
  if(!dec || dec.action !== "trade"){
    return `
      <div class="card">
        <div class="kicker">LOCKED</div>
        <div class="h2">Decision required</div>
        <div class="sub">Go Home → choose <b>Trade</b> (ritual + commit), then log.</div>
        <div class="row" style="margin-top:12px">
          <button class="btn" onclick="location.hash='home'">Back</button>
        </div>
      </div>
  ...
  // Keep actual entry logging inside the strategy window.
  if(!win.inWindow){
    return `
      <div class="card">
        <div class="kicker">LOCKED</div>
        <div class="h2">Outside trade window</div>
        <div class="sub">Entry logging is limited to the strategy window.</div>
        <div class="row" style="margin-top:12px">
          <button class="btn" onclick="location.hash='home'">Back</button>
        </div>
      </div>
...
function makeDayDecision(type){
  const rth = nowInRTHSessionET();
  if(!rth.inSession){ toast("Outside RTH — no decisions/logging now."); return; }
  const win = nowInTradeWindowET();
  const ritualDone = (state.ritual && state.ritual.lastDone === todayStr());
  ...
  if(type==="skip"){
    // reward compliance
    ensureBehaviorDays();
    state.behavior.days[todayStr()] = "green";
    saveState();
    toast("Skip locked ✅");
    render(route());
  }else{
...
// Gate access to New Trade so you can't bypass the ritual/decision flow.
function attemptNewTrade(){
  const win = nowInTradeWindowET();
  const dec = (state.dayDecision && state.dayDecision.date === todayETStr()) ? state.dayDecision : null;

  if(!win.isWeekday){
    toast("Locked: weekend");
    return;
  }
  if(!win.inSession){
    toast("Locked: outside RTH");
    location.hash = "#outside";
    return;
  }
  if(!win.inWindow){
    toast("Locked: outside trade window");
    location.hash = "#outside";
    return;
  }
  if(!dec){
    toast("Make today's decision first");
    location.hash = "#home";
    return;
  }
  if(dec.action !== "trade"){
    toast("You chose SKIP today");
    location.hash = "#home";
    return;
  }
  // If ritual isn't done for today, force it before trade logging.
  if(ritualRequiredNow()){
    openRitual('newtrade');
    return;
  }
  location.hash = "#newtrade";
...
function ritualCard(){
  ...
  const label = doneToday ? "Ritual Done ✅" : "Bank Ritual (3m)";
    ...
    <div class="card">
      <div class="row" style="justify-content:space-between;align-items:center;">
        <div>
          ...
          <div class="muted">3‑minute conditioning.</div>
        </div>
...
function openRitual(nextHash){
  const win = nowInTradeWindowET();
  if(!win.inWindow){ toast('Outside trade window (09:30–11:30 ET).'); return; }
  state.ritual = state.ritual || {};
  state.ritual.active = true;
  state.ritual.t = 180; // 3 minutes
  state.ritual.phase = "RESET";
  state.ritual.pendingHash = nextHash || state.ritual.pendingHash || null;
  saveState();
  render(route());
  startRitualTimer();
}
...
function startRitualTimer(){
  window.__ritualTimer && clearInterval(window.__ritualTimer);
  const phases = [
    {name:"RESET", dur:75, mantra:["Exhale long.","Shoulders down.","Eyes soft."]},
    {name:"IDENTITY", dur:45, mantra:["I trade like a bank.","One token. One decision.","Boring = profitable."]},
    {name:"RISK", dur:45, mantra:["6 points risk.","9 points target.","No chase. No add."]},
    {name:"COMMIT", dur:15, mantra:["Wait for signal.","Act once.","Protect capital."]}
  ];
  let idx=0, remain=phases[0].dur;

  const paint=()=>{
    const p = Math.min(100, Math.round((180 - state.ritual.t)/1.8));
    const prog=document.getElementById("ritualProg");
    if(prog) prog.style.setProperty("--p", String(p));
    const w=document.getElementById("ritualPhase");
    if(w) w.textContent = phases[idx].name;
    const m=document.getElementById("ritualMantra");
    if(m){
      const mi = Math.floor((180 - state.ritual.t) / 5) % phases[idx].mantra.length;
      m.textContent = phases[idx].mantra[mi];
    }
    const n=document.getElementById("ritualNum");
    if(n) n.textContent = Math.max(0,state.ritual.t);
    const c=document.getElementById("ritualCommit");
    if(c) c.style.display = (state.ritual.t<=2)?"inline-flex":"none";
  };
  paint();

  window.__ritualTimer=setInterval(()=>{
    if(!state.ritual.active){clearInterval(window.__ritualTimer);return;}
    state.ritual.t--;
    remain--;
    if(remain<=0 && idx<phases.length-1){
      idx++; remain=phases[idx].dur;
      state.ritual.phase=phases[idx].name;
    }
    saveState(); paint();
    if(state.ritual.t<=0){ clearInterval(window.__ritualTimer); }
  },1000);
}
...
function ritualCommit(){
  ...
  state.ritual.date = todayKeyET();
  saveState();
  toast("Bank Mode primed ✅");
  // If the ritual was triggered as a gate (e.g. +Trade), continue automatically.
  const next = state.ritual.pendingHash;
  state.ritual.pendingHash = null;
  saveState();
  if(next){ location.hash = next.replace(/^#/, ''); return; }
  render(route());
}



function isDayClosedToday(){
  return state.dayClosed && state.dayClosed.date === todayETStr();
}

function openDebrief(){
  state.debrief = { active:true, t:60, phase:0, follow:null };
  saveState();
  render(route());
  startDebriefTimer();
}
...
function startDebriefTimer(){
  ...
  const paint=()=>{
    const n=document.getElementById("debriefNum");
    if(n) n.textContent = String(Math.max(0,state.debrief.t));
    const phase=document.getElementById("debriefPhase");
    if(phase){
      phase.textContent = state.debrief.phase===0 ? "EXHALE" : (state.debrief.phase===1 ? "ACCOUNT" : "CLOSE");
    }
    const qa=document.getElementById("debriefQA");
    const seal=document.getElementById("debriefSeal");
    if(qa) qa.style.display = (state.debrief.phase===1) ? "block" : "none";
    if(seal) seal.style.display = (state.debrief.phase===2) ? "inline-flex" : "none";
  };
  paint();
  window.__debriefTimer=setInterval(()=>{
    if(!state.debrief || !state.debrief.active){ clearInterval(window.__debriefTimer); return; }
    state.debrief.t = Math.max(0, state.debrief.t - 1);
    // phases: 0-20 exhale, 21-40 account, 41-60 close
    const elapsed = 60 - state.debrief.t;
    state.debrief.phase = elapsed < 20 ? 0 : (elapsed < 40 ? 1 : 2);
    saveState();
    paint();
    if(state.debrief.t<=0){
      clearInterval(window.__debriefTimer);
      // keep on CLOSE until user taps DAY CLOSED
      state.debrief.phase = 2;
      saveState();
      paint();
    }
  },1000);
...
function setDebriefFollow(v){
  state.debrief.follow = v;
  // feed behavior
  ensureBehaviorDays();
  state.behavior.days[todayETStr()] = v ? "green" : "red";
  saveState();
  toast(v ? "Rules: YES" : "Rules: NO");
  render(route());
}

function closeDay(){
  // require follow selection? no, but prefer. we'll allow.
  state.dayClosed = { date: todayETStr() };
  state.debrief.active = false;
  saveState();
  toast("Bank closed.");
  render(route());
}
...
function debriefModal(){
  ...
  const p = Math.min(100, Math.round((60 - state.debrief.t) * (100/60)));
  return `
    <div class="modal" onclick="">
      <div class="modalCard" onclick="event.stopPropagation()">
        <div class="ritualProg" style="--p:${p};">
          <div class="ritualInner">
            <div class="phaseWord" id="debriefPhase">EXHALE</div>
            <div class="ritualNum" id="debriefNum">60</div>
          </div>
        </div>

        <div id="debriefQA" style="display:none;margin-top:12px;">
          <div class="muted" style="margin-bottom:8px;">Followed rules?</div>
          <div class="row" style="gap:10px;flex-wrap:wrap;">
            <button class="btn btn-primary" onclick="setDebriefFollow(true)">Yes ✅</button>
            <button class="btn btn-danger" onclick="setDebriefFollow(false)">No ❌</button>
          </div>
        </div>

        <button id="debriefSeal" class="btn btn-primary" style="display:none;margin-top:12px;" onclick="closeDay()">DAY CLOSED</button>
      </div>
    </div>`;
}
