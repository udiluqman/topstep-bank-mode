...
function loadState(){
  ...
  try {
    ...
    // migrations
    if(!s.news) s.news = {bufferMin: 10, events: [], apiUrl: '', lastSyncET: '', autoEnabled: true};
...
function ritualRequiredNow(){
  ...
  const done = state.ritual && state.ritual.date === todayKeyET();
  return !done;
}
    ...
    <div class="modalCard" onclick="event.stopPropagation()">
      <div class="ritualProg" id="ritualProg">
        <div class="ritualInner">
          <div class="phaseWord" id="ritualPhase">RESET</div><div class="muted" style="font-size:12px;margin-top:-2px;">Breathe. Shoulders down.</div>
...
function openRitual(){
  const rth = nowInRTHSessionET();
  if(!rth.inSession){ toast('Outside RTH — come back at 09:30–16:00 ET.'); return; }
...
function startRitualTimer(){
  window.__ritualTimer && clearInterval(window.__ritualTimer);
  const phases = [
    {name:"RESET", dur:75}, // breathing
    {name:"IDENTITY", dur:45},
    {name:"RISK", dur:45},
    {name:"COMMIT", dur:15}
  ];
  ...
  const paint=()=>{
    ...
    if(c) c.style.display = (state.ritual.t<=15)?"inline-flex":"none";
  };
  paint();
...
function ritualCommit(){
  ...
  state.ritual.lastDone = todayStr();
  saveState();
